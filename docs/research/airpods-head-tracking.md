...
**(a) Native macOS helper — recommended.** Tiny Swift menu-bar (or LSUIElement) app on macOS 14+: `CMHeadphoneMotionManager` → project accel onto gravity → stream samples to the Node server on localhost → page consumes via the SSE channel. **Inbound contract (new work — today's `/events` is outbound-only and nothing ingests motion):** a new `POST /api/motion` endpoint accepting batched samples `{"samples": [{"t": <ms>, "av": <vertical accel, g>, "pitch": <deg>, "loc": "left"|"right"|"default"}]}` — `samples` is the **only** field the server reads. Any other top-level field, `session` included, is ignored rather than rejected: tolerating them keeps a newer helper compatible with an older server, and ignoring them is what stops a client influencing a value the server stamps itself.

Details:

- **Envelope.** Always the `samples` object — never a bare array or bare sample — so a batch of one and a batch of fifty parse identically. The server re-broadcasts **one SSE `motion` event per accepted sample**, so batching is purely a transport optimisation and the adapter never sees batch boundaries. Each event's `data` is one sample plus the server's `session`, and nothing else: `{"session": "<id>", "t": <ms>, "av": <g>, "pitch": <deg>, "loc": "left"|"right"|"default"}`. Field order is not guaranteed — it is JSON, so the adapter reads by name and validates this shape before use rather than trusting the bus.
- **Validation, before anything is broadcast.** Strict JSON types, with units and the bounds that are knowable without hardware fixed here: `t` a **non-negative integer** of milliseconds; `pitch` in **degrees**, inclusive `[-180, 180]`, which is definitional rather than measured; `av` in **g**, finite, with its inclusive bound taken from the accelerometer's full-scale range — that one figure needs the device spec, so it is named at implementation rather than guessed at here. All three must be finite: `NaN` and `±Infinity` are rejected, since JSON will not carry them but a lenient parser can still produce them. A maximum body size and a maximum `samples` length, both enforced *while reading* rather than after, so an oversized body is abandoned mid-stream instead of being buffered. Oversized bodies get `413`; malformed, out-of-range or empty-`samples` payloads get `400` — distinct codes, because "you sent too much" and "you sent nonsense" need different fixes. **A batch is atomic:** one invalid sample rejects the whole batch and nothing from it is broadcast, so a malformed tail cannot half-apply or poison the detector. The two numeric caps are deliberately left to implementation: they follow from the helper's real batch cadence, which is not measured yet, and inventing figures here would give them false authority. Pick them with the sampling rate in hand, and state them in the same commit as the endpoint. **Until those two figures are chosen and tested, treat this contract as provisional** — a helper cannot size its batches and a client cannot reliably interpret a `413` without them, so the endpoint is not implementable from this document alone, by design.
- **Trust boundary.** The endpoint is state-changing, and localhost is *not* a boundary: a public page can issue a simple cross-origin `POST` to `127.0.0.1` that is not preflighted, so no CORS policy is consulted before the write lands. Current browsers are closing this off — Private Network Access sends a preflight for public→loopback requests — but that is recent, uneven across browsers and versions, and not something a local server should rely on being enforced. DNS rebinding is the complementary trick: it does not change the page's origin, it changes what that origin *resolves to*, so a same-origin request from the attacker's own page arrives at loopback. And every local process can post regardless of any of this. So the server binds loopback only (as `server.js` already does) and, **before it reads a body**, requires `Authorization: Bearer <token>` and rejects any request whose `Origin` header is present and is not one of the app's own two spellings — `http://127.0.0.1:<port>` and `http://localhost:<port>`, both allowed because either can be typed into the address bar and `server.js` answers on both. Every other origin is refused.
- **Token handoff.** Minted by the server at startup from a CSPRNG (`crypto.randomBytes`), at least 128 bits, never a fixed default and never derived from the port, PID or clock. Compared as a whole value in constant time, so a wrong token cannot be narrowed by timing. It is never logged, never echoed in an error body, and never placed in the environment. It must not travel in `argv` — process arguments are world-readable via `ps` on a shared machine. Pass it on an inherited file descriptor, or write it `0600` under `$PROMPTUPS_DATA_DIR` for the helper to read.
- **Session boundary.** The **server** is the only component that mints a `session` identifier — at startup, alongside the token — and it stamps that value onto every rebroadcast `motion` event itself. A `session` field in an inbound `POST` is not trusted: the server ignores it and stamps its own, so a client cannot force a detector reset by claiming a new session, and a stale helper posting a previous session is simply re-stamped rather than honoured (its token will have been rotated at the same restart, so it fails auth first). `t` is the Core Motion sample timestamp ([monotonic seconds since boot](https://developer.apple.com/documentation/coremotion/cmlogitem/timestamp)), converted to ms **once in the helper** and preserved unchanged through rebroadcast — the server never substitutes receive time. Its origin is the **boot**, so it survives a helper restart unchanged and resets only on reboot — a restarted helper keeps emitting from the same clock, and nothing rebases it. That is exactly why the adapter cannot detect a discontinuity from the timestamps alone: after a server restart the stream resumes with plausible, still-increasing `t` values while the baseline behind them is stale. The `session` identifier is the signal instead — the adapter resets whenever it changes. That covers exactly the cases that rotate it: a reboot and a server restart. It does **not** cover a gap with the server still up (a sleeping laptop, a dropped helper that reconnects, a stalled SSE bus): the session is unchanged, so the adapter sees a clean continuation of a stale baseline. Closing that needs a separate staleness rule — a max age on the previous sample, or a heartbeat — which is deliberately not specified here because it belongs with the measured sampling cadence.
